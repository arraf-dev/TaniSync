...
<header class="glass-panel sticky top-0 z-30 flex h-20 items-center justify-between border-b border-[#cad4c4]/70 px-5 md:px-8">
    <div>
        <p class="text-xs font-bold uppercase tracking-[0.2em] text-[#9c5421]">{{ $roleLabel }}</p>
    ...
    <div class="flex items-center gap-3">
        ...
        <div class="flex items-center gap-3 rounded-full border border-[#cad4c4]/70 bg-white px-3 py-2">
            <img src="{{ $role === 'admin' ? 'https://i.pravatar.cc/120?img=12' : ($role === 'superadmin' ? 'https://i.pravatar.cc/120?img=3' : 'https://i.pravatar.cc/120?img=15') }}" alt="{{ $user?->name }}" class="h-10 w-10 rounded-full object-cover">

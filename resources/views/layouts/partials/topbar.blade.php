<header class="topbar-shell">
    <div class="min-w-0">
        <div class="flex items-center gap-2">
            <span class="status-pill {{ $role === 'super_admin' ? 'status-muted' : ($role === 'admin' ? 'status-warning' : 'status-success') }}">
                {{ $role === 'super_admin' ? 'Area platform' : ($role === 'admin' ? 'Area admin' : 'Area petani') }}
            </span>
            @if ($user?->organization?->name)
                <span class="hidden text-xs font-semibold text-[#718174] sm:inline">{{ $user->organization->name }}</span>
            @endif
        </div>
    ...
    <div class="flex items-center gap-3 rounded-2xl border border-[#dfe8dc] bg-white px-3 py-2">
        <div class="hidden text-right sm:block">
            ...
            <p class="text-xs text-[#718174]">{{ $role === 'super_admin' ? 'Pengelola platform' : ($role === 'admin' ? 'Pengelola organisasi' : 'Petani terdaftar') }}</p>
        </div>
